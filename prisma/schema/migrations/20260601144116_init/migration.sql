...
CREATE TABLE `ModulePermission` (
    ...
    `accessScope` ENUM('ALL', 'OWN', 'BRANCH', 'STORE') NOT NULL DEFAULT 'ALL',
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
...
CREATE TABLE `RolePermission` (
    ...
    `modulePermissionId` VARCHAR(191) NOT NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,
    `deletedAt` DATETIME(3) NULL,
...
-- AddForeignKey
ALTER TABLE `ModulePermission` ADD CONSTRAINT `ModulePermission_permissionId_fkey` FOREIGN KEY (`permissionId`) REFERENCES `Permission`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
