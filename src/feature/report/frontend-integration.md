...
1. **Sales & Revenue**: Completed paid orders, gross revenue, discounts, net sales, and average ticket size.
...
- **Query Parameters**:
  ...
  - `type` (optional enum):
    ...
    - *(omitted)*: Returns the complete, compiled dataset with all sections.
...
- **Export File**: `POST /api/reports/export`
  - Body: `{ "module": "financials", "filters": { "dateFrom": "2026-08-01", "dateTo": "2026-08-31" }, "format": "excel" }` *(or `"pdf"`)*
