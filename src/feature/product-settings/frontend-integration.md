...
Used to categorize items (e.g. Espresso, Waffles, Pasta, Matcha Series). Each category can optionally be linked to a parent `ProductType` (_Food_ or _Beverage_).

- **`GET /product-settings/categories`**: Paginated and searchable list of categories.
    - **RBAC**: `read`
    - **Query Parameters**: Common parameters (`page`, `limit`, `search`, `status`) + `productTypeId` (string, UUID, optional) to filter categories by their parent type.
- **`GET /product-settings/categories/:id`**: Retrieves a specific category profile, including its parent `type` object.
    - **RBAC**: `read`
- **`POST /product-settings/categories`**: Creates a new product category.
    - **Body**: `name` (string, required, min 2, max 100), `description` (string, optional, max 500), `productTypeId` (string, UUID, optional).
    - **RBAC**: `create`
- **`PUT /product-settings/categories/:id`**: Updates category fields.
    - **Body**: `name` (optional), `description` (optional), `productTypeId` (string, UUID, optional).
    - **RBAC**: `update`
...
Used to declare item types (e.g. `Beverage`, `Food`).

- **`GET /product-settings/types`**: Paginated and searchable list of product types, including their active child `categories` array.
    - **RBAC**: `read`
- **`GET /product-settings/types/:id`**: Specific product type details, including its active child `categories` array.
    - **RBAC**: `read`
