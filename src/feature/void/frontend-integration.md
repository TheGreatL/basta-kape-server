...
- **Role/Permission Requirement**: The override user (`actorId` from Bearer token) must be assigned a role holding `delete` permission for the `Point of Sale (POS)` module.
- **Request Headers**:
    ```http
...
- **Error Responses**:
    - **400 Bad Request**:
        ...
        - _Example_:
            ...
            ```
    - **403 Forbidden**:
        - Reason: The user does not have `POINT_OF_SALE` `DELETE` access scope, OR the override user does not possess `delete` permission for the `Point of Sale (POS)` module.
        - _Example_:
            ```json
            { "error": "Insufficient permissions. Delete permission on Point of Sale (POS) module required." }
            ```
    - **404 Not Found**:
...
1. **Manager Override Modal**:
   When a cashier clicks the **Void** button on an order:
    - Open a dialog prompting for override credentials of an authorized user who has `delete` permission on `Point of Sale (POS)` (if the currently logged-in cashier does not have this permission).
    - Submit the void POST request using that authorized user's credentials/token.
