...
- [ ] Define schemas & Zod validation types (`order.types.ts`)
- [ ] Create repository queries (`order.repository.ts`)
- [ ] Implement ordering and status management business logic (`order.service.ts`)
- [ ] Declare API routes (`GET /`, `GET /:id`, `POST /`, `PATCH /:id/status`) & Swagger docs (`order.route.ts`)
- [ ] Write ordering flow integration tests (`order.test.ts`)
